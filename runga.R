...
runGA <- function(noRuns = 30, problem = "feature"){
  ...
  if (problem == "feature"){
    maxGenerations <<- 10    #<<- makes it a global variable. So it will be visible to other functions e.g. monitor()
    popSize = 50
    pcrossover = 0.8
    pmutation = 0.1
    type = "binary"
  ...
  for (i in 1:noRuns){
    ...
    if (problem == "feature")
      GA <- ga(type=type, fitness = fitness, xx=xx, yy=yy, nBits = ncol(xx), names = colnames(xx), seed=i, popSize = popSize, 
               pcrossover = pcrossover, pmutation = pmutation, maxiter = maxGenerations, monitor= monitor )
    else if (problem == "tsp")
